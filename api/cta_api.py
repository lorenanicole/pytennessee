...
class CTABustracker(object):
    ...
    def get_predictions_for_stops(self, requested_time, route_id, stp_id, top=3):
        base_url = "http://www.ctabustracker.com/bustime/api/v1/getpredictions?key={0}&rt={1}&stpid={2}&top={3}"
        try:
            response = requests.get(base_url.format(self.key, requested_time, route_id, stp_id, top))
            return [BusPrediction(requested_time=requested_time, **prediction) for prediction in xmltodict.parse(response.content).get("bustime-response").get("prd")]
        except Exception as e:
            logger.info("Exception: " + str(e.message))

    def get_raw_predictions_for_stops(self, route_id, stp_id, top=3):
        base_url = "http://www.ctabustracker.com/bustime/api/v1/getpredictions?key={0}&rt={1}&stpid={2}&top={3}"
        try:
            response = requests.get(base_url.format(self.key, route_id, stp_id, top))
            return [prediction for prediction in xmltodict.parse(response.content).get("bustime-response").get("prd")]
        except Exception as e:
            logger.info("Exception: " + str(e.message))
...
class CTATraintracker(object):
    ...
    def get_predictions_for_stops(self, requested_time, stp_id, max=10):
        ...
        try:
            ...
            return [TrainPrediction(requested_time=requested_time, **prediction) for prediction in xmltodict.parse(response.content).get("ctatt").get("eta")]
        except Exception as e:
            logger.info("Exception: " + str(e.message))