...
class WeatherUnderground(object):
    ...
    def get_current_weather_for_city(self, requested_time, city, state):
        request_url = self.BASE_URL.format(self.key) + "conditions/q/{0}/{1}.json".format(state, city)
        return CurrentWeather(requested_time=requested_time, **json.loads(requests.get(request_url).content).get("current_observation"))