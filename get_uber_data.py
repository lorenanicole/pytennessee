...
if 6 < datetime.now().hour < 12:
    ...
    uber_eta_predictions = uber.get_ride_arrival_time(request_id=request_id, starting_lat=41.908511, starting_long=-87.696287)
    uber_duration_predictions = uber.get_ride_duration_and_price(request_id=request_id,
                                                                 starting_lat=41.908511,
                                                                 starting_long=-87.696287,
                                                                 ending_lat=41.879931,
                                                                 ending_long=-87.629024)
elif 15 < datetime.now().hour < 20:
    logger.info("Request ID: " + str(request_id) + " running for afternoon work uber")
    uber_eta_predictions = uber.get_ride_arrival_time(request_id=request_id, starting_lat=41.879931, starting_long=-87.629024)
    uber_duration_predictions = uber.get_ride_duration_and_price(request_id=request_id,
                                                                 starting_lat=41.879931,
                                                                 starting_long=-87.629024,
                                                                 ending_lat=41.908511,
